...
export default function InspeksiIcsPage() {
  ...
  const downloadPdf = async () => {
    if (!savedRecordId || pdfLoading) return;
    setPdfLoading(true);
    try { await downloadAppoliPdf('inspeksiICS', savedRecordId); }
    catch (error) { setMessage(error instanceof Error ? error.message : 'PDF tidak dapat diunduh.'); }
    finally { setPdfLoading(false); }
  };
